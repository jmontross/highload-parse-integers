...
## Champion
- **dp2_8s_fw_t0_2048 (RE-PROMOTED 2026-07-15, current — ×146, confirmed STOP-FLOOR ×146)** — `double-loop + T0@512B (8 iters, L2→L1) + T1@2048B (32 iters, DRAM→L2) per stream; single u16 accumulator`
  — STOP-FLOOR ×146 (2026-07-15, RUNS=3, floor=0.240s medium VM): champion best=0.0800s, median=0.0800s; best variant (dp2_8s_fw_t0_128_3072) best=0.0780s but median=0.0830s > 0.0800s → HOLD → STOP-FLOOR ×146. Edge 9/9. All 165 cpp variants (1 WRONG: dp2_8s_u8tree) + 1 rs. Compiler sweep: **g++ -O3 -march=native → 0.0790s best** (vs g++-13 → 0.0800s, clang++ → 0.0880s). index.html: 80ms. No new variants — all angles definitively exhausted. Algorithm at bandwidth ceiling for 146 consecutive STOP-FLOOR runs. **SUBMIT `champion/main.cpp` with `g++ -O3 -march=native`.** Expected judge time: ~55-65ms.
