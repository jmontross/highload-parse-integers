...
- **dp2_8s_fw_t0_64_3072 (PROMOTED 2026-07-30 at ×308; T0@64B/1-iter near + T1@3072B/48-iters far)** — `gate run: best=0.070s vs prior champion (dp2_8s_fw_t0_256) 0.073s (4.1% margin), median 0.074s < 0.077s champion, 9/9 edge → PROMOTE`
  — VM oscillation cascade ×308 (2026-07-30, RUNS=5, floor=0.492s slow VM): Stabilization run with champion=dp2_8s_fw_t0_256 (remote run ×307). STOP-FLOOR confirmed (0.073s << 2×0.492s). PROMOTE fired for dp2_8s_fw_t0_64_3072: best=0.070s (need ≤0.0719s ✓) median=0.074s < champion 0.077s ✓; Δbest=4.1%; edge 9/9 → PROMOTED. Classic VM oscillation: all dp2 variants cluster 0.070-0.083s; which wins rotates with VM microstate (documented ×212-×307). No new algorithm code created — all 181 cpp + 1 rs variants exhausted. Design space fully saturated. **SUBMIT `champion/main.cpp` with `g++ -O3 -march=native`.** Expected judge time: ~55-65ms on bare metal. index.html: 73ms (slow VM, 1.1× off rank-18 bar ≤69.3ms).
