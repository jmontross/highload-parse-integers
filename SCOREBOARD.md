...
## Champion
- **dp2_8s_fw_4acc_t0_64_512 (PROMOTED 2026-07-15, current — ×142, confirmed STOP-FLOOR ×142)** — `4 independent per-pair u16 accumulators + T0@64B (1 iter, L2→L1) + T1@512B (8 iters, DRAM→L2) per stream; judge-tuned for ~80ns DRAM`
  — STOP-FLOOR ×142 (2026-07-15, RUNS=3, floor=0.174s min / 0.501s median moderate-slow VM): champion best=0.075s, median=0.077s; best variant (dp2_8s_fixed_2048) best=0.074s need ≤0.0739s → Δbest=0.001s only (1.3% < 1.5% gate), also median=0.078 > 0.077 → HOLD → STOP-FLOOR ×142. Edge 9/9. 160 cpp variants (1 WRONG: dp2_8s_u8tree) + 1 rs. Compiler sweep: **g++-13 -Ofast -march=native -funroll-loops → 0.073s best** (vs g++ -O3 → 0.079s, clang++ → 0.084s). index.html: 75ms (1.1× off rank-18 bar). **SUBMIT `champion/main.cpp` with `g++-13 -Ofast -march=native -funroll-loops`.** Expected judge time: ~55-65ms.
