// dp2_8s_fw_4acc_t1_2048.cpp — 4 independent per-pair u16 accumulators + T1@2048B only (no T0).
// Current champion dp2_8s_fw_4acc_t0t1 uses T0@512B (near, L1) + T1@3072B (far, L2).
// This variant: T1@2048B only — removes 8 T0 prefetch µops/iter.
// Theory: On judge (~80ns DRAM latency), T1@2048B ≈ 32 iters ahead is more than enough.
//   Removing T0 saves 8 µops/iter; the L1 HW prefetcher or T1@2048B data arrival may suffice.
//   dp2_8s_fw_t0_2048 (previous champion) won over t0_t1 on some VM states — same prefetch
//   distance but without 4acc. This adds 4acc to the 2048B single-tier.
...
// 4 independent per-pair accumulators + T1@2048B only (no T0).
