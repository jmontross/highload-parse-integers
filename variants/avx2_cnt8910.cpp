// HighLoad.fun — parse_integers  (VARIANT: avx2_cnt8910)
// Extends avx2_parse_quad with explicit fast paths for cnt==8, 9, 10.
//
// MOTIVATION:
//   avx2_parse_quad handles cnt==4-7 via parse_quad/pair, but cnt>=8 falls
//   to the while(m) fallback which calls parse_num individually per number.
//   With avg line length ~10 bytes and 64-byte scan window:
//     P(cnt=8) ≈ 10.5%,  P(cnt=9) ≈ 7.4%,  P(cnt=10) ≈ 4.7%
//   ~22% of iterations currently use the fallback. New fast paths:
//     cnt=8  → 2 × parse_quad
//     cnt=9  → 2 × parse_quad + 1 × parse_num
//     cnt=10 → 2 × parse_quad + 1 × parse_pair
//   On the judge (compute-bound), this eliminates per-number parse_num overhead
//   for these cases, replacing ~15 cycles/number with ~10 cycles/number.
//
// All cnt paths use the same pointer arithmetic as avx2_parse_quad to
// correctly handle base < p (numbers spanning 64-byte window boundaries).
#include <cstdio>
#include <cstdint>
#include <cinttypes>
#include <cstring>
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    while (p < safe_end) {
        ...
        if (__builtin_expect(cnt == 6, 1)) {
            ...
            // 2 × parse_quad covers all 8 numbers
            uint64_t mm = m;
