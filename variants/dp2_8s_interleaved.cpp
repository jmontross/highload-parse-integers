// dp2_8s_interleaved.cpp — dp2_8s_stop_pf3072 with interleaved mask+process per stream.
// Change: instead of (prefetch×8 → nl_mask64×8 → process_window×8), do
// (prefetch×8 → per-stream: nl_mask64, process_window) sequentially.
// Theory: reduces peak live register count (m_i freed before m_{i+1} computed),
// gives OOO engine compute work to overlap with next stream's load latency.
// This interleaved pattern was never tested in the dp2 era (only in the avx2 era
// where it was champion; dp2's independent per-stream base pointers make it
// structurally different — worth measuring).
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        // Issue all 8 prefetches first (maximum time before first access),
        // then interleave mask+process per stream: reduces peak live registers
        // (m_i freed before m_{i+1} computed) while preserving prefetch MLP.
        for (size_t n = safe_iters; n-- > 0; ) {
            ...
            // Interleaved: compute mask then process window per stream.
            // Each (nl_mask64, process_window) pair is independent; the OOO engine
            // can overlap AVX2 loads (ports 2/3) with integer parse (ports 0/1/5)
            // across adjacent stream pairs.
            __m128i r0 = process_window_dp(p0, b0, nl_mask64(p0)); p0 += 64;
            __m128i r1 = process_window_dp(p1, b1, nl_mask64(p1)); p1 += 64;
            __m128i r2 = process_window_dp(p2, b2, nl_mask64(p2)); p2 += 64;
            __m128i r3 = process_window_dp(p3, b3, nl_mask64(p3)); p3 += 64;
            __m128i r4 = process_window_dp(p4, b4, nl_mask64(p4)); p4 += 64;
            __m128i r5 = process_window_dp(p5, b5, nl_mask64(p5)); p5 += 64;
            __m128i r6 = process_window_dp(p6, b6, nl_mask64(p6)); p6 += 64;
            __m128i r7 = process_window_dp(p7, b7, nl_mask64(p7)); p7 += 64;
