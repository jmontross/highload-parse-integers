// dp2_8s_triple_pair.cpp — dp2_8stream with 3-window u8 grouping before u16 widening.
// Current champion groups 2 windows per acc_u16_add (4 calls per iter: r0+r1, r2+r3, r4+r5, r6+r7).
// This variant groups 3+3+2 = 3 calls per iter: (r0+r1+r2), (r3+r4+r5), (r6+r7).
// Safety: max u8 per triple = 3 × 9 × 8 = 216 ≤ 255 (same overflow profile as paired).
// Saves 1 VPMOVZXBW + 1 VPADDW + 1 _mm_add_epi8 per main-loop iteration.
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        while (__builtin_expect(p0 < s0 & p1 < s1 & p2 < s2 & p3 < s3 &
            ...
            // 3+3+2 grouping: saves 1 VPMOVZXBW + 1 VPADDW per iteration vs 2+2+2+2.
            acc_u16_add(acc_u16, _mm_add_epi8(_mm_add_epi8(r0, r1), r2));
            acc_u16_add(acc_u16, _mm_add_epi8(_mm_add_epi8(r3, r4), r5));
