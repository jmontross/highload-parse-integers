// avx2_pdep: champion avx2_parse_quad but replaces the serial CTZ+BLSR chain
// with parallel PDEP extractions.
//
// MOTIVATION:
//   For cnt==6, the current champion does 6 serial iterations of:
//     n_i = ctz(mm); mm = blsr(mm);
//   Each blsr has ~1cy latency, ctz has ~3cy → serial chain = 6×4 = 24 cycles.
//
//   With PDEP (BMI2), all 6 bit positions are extracted INDEPENDENTLY from m:
//     n0 = ctz(pdep(0x01, m))  ← depends only on m (not n0, n1, ...)
//     n1 = ctz(pdep(0x02, m))  ← same, all parallel
//     ...
//   PDEP latency = 3cy + CTZ = 3cy → total latency for ALL 6 = 6 cycles.
//   Saves ~18 cycles per 64-byte window = ~1.8ns/window at 3GHz.
//   Applied to all cnt fast paths (4,5,6,7) and the while(m) fallback.
//
// Everything else identical to avx2_parse_quad.
#include <cstdio>
#include <cstdint>
#include <cinttypes>
#include <cstring>
...
// Extract k-th set bit position using PDEP: all calls independent, 3+3=6cy latency total.
// On Intel BMI2 CPUs: PDEP latency = 3cy, TZCNT latency = 3cy.
// vs serial blsr+ctz chain: 6 × 4cy = 24cy latency.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    while (p < safe_end) {
        ...
        if (__builtin_expect(cnt == 6, 1)) {
            // All 6 positions extracted in PARALLEL via PDEP (no serial chain)
            unsigned n0 = NTH_BIT(0, m);
            unsigned n1 = NTH_BIT(1, m);
            unsigned n2 = NTH_BIT(2, m);
            unsigned n3 = NTH_BIT(3, m);
            unsigned n4 = NTH_BIT(4, m);
            unsigned n5 = NTH_BIT(5, m);
