// dp2_16s_fw_t0_64_512.cpp — 16 spatially-separated streams, same digit-place
// algorithm as champion dp2_8s_fw_t0_64_512 but with double the MLP.
//
// Motivation: judge DRAM latency ~80ns / cycle_time ~33cy/iter = need ~7.3 streams
// to fully hide latency. 8 streams is barely enough. On slower DRAM (100ns) we'd
// need 9+ streams. 16 streams provides a safety margin and tests whether additional
// outstanding DRAM requests improve throughput. dp2_12s was tried; dp2_16s is new.
//
// Trade-off: 16 stream pointers + 16 base pointers may cause register spilling.
// We process two groups of 8 streams per iteration to amortize the widen overhead,
// but keep the inner loop compact to minimize spill cost.
...
// 16 streams: T0@64B (just-in-time L1 warm) + T1@512B (covers 80ns DRAM latency).
// 16 streams × 2 prefetches = 32 prefetch µops per iteration.
// Process in two sub-groups of 8 streams to keep register pressure manageable.
#define ITER_BODY_A(PFD) \
...
#define ITER_BODY_B(PFD) \
    _mm_prefetch((const char*)(p8 + 64), _MM_HINT_T0); \
    _mm_prefetch((const char*)(p8 + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p9 + 64), _MM_HINT_T0); \
    _mm_prefetch((const char*)(p9 + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(pa + 64), _MM_HINT_T0); \
    _mm_prefetch((const char*)(pa + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(pb + 64), _MM_HINT_T0); \
    _mm_prefetch((const char*)(pb + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(pc + 64), _MM_HINT_T0); \
    _mm_prefetch((const char*)(pc + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(pd + 64), _MM_HINT_T0); \
    _mm_prefetch((const char*)(pd + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(pe + 64), _MM_HINT_T0); \
    _mm_prefetch((const char*)(pe + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(pf + 64), _MM_HINT_T0); \
    _mm_prefetch((const char*)(pf + (PFD)), _MM_HINT_T1); \
    { \
    uint64_t m8 = nl_mask64(p8); \
    uint64_t m9 = nl_mask64(p9); \
    uint64_t ma = nl_mask64(pa); \
    uint64_t mb = nl_mask64(pb); \
    uint64_t mc = nl_mask64(pc); \
    uint64_t md = nl_mask64(pd); \
    uint64_t me = nl_mask64(pe); \
    uint64_t mf = nl_mask64(pf); \
    __m128i r8 = process_window_dp(p8, b8, m8); p8 += 64; \
    __m128i r9 = process_window_dp(p9, b9, m9); p9 += 64; \
    __m128i ra = process_window_dp(pa, ba, ma); pa += 64; \
    __m128i rb = process_window_dp(pb, bb, mb); pb += 64; \
    __m128i rc = process_window_dp(pc, bc, mc); pc += 64; \
    __m128i rd = process_window_dp(pd, bd, md); pd += 64; \
    __m128i re = process_window_dp(pe, be, me); pe += 64; \
    __m128i rf = process_window_dp(pf, bf, mf); pf += 64; \
    acc_u16_add(acc_u16, _mm_add_epi8(r8, r9)); \
    acc_u16_add(acc_u16, _mm_add_epi8(ra, rb)); \
    acc_u16_add(acc_u16, _mm_add_epi8(rc, rd)); \
    acc_u16_add(acc_u16, _mm_add_epi8(re, rf)); \
    }
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        const unsigned char* adj_start[16];
        const unsigned char* adj_end[16];

        adj_start[0] = data;
        for (int i = 1; i < 16; i++) {
            const unsigned char* q = data + (size_t)i * (size / 16);
            while (q < end && *q != '\n') ++q;
            adj_start[i] = (q < end) ? q + 1 : end;
        }
        for (int i = 0; i < 15; i++) adj_end[i] = adj_start[i + 1];
        adj_end[15] = end;
        ...
        const unsigned char *pa=adj_start[10], *ba=adj_start[10];
        const unsigned char *pb=adj_start[11], *bb=adj_start[11];
        const unsigned char *pc=adj_start[12], *bc=adj_start[12];
        const unsigned char *pd=adj_start[13], *bd=adj_start[13];
        const unsigned char *pe=adj_start[14], *be=adj_start[14];
        const unsigned char *pf=adj_start[15], *bf=adj_start[15];

        __m256i acc_u16 = _mm256_setzero_si256();

        // Inner loop: 50 iterations per group (vs champion's 100).
        // 16 streams × 4 acc_u16_add per iter: max u16 contribution per iter = 4×144=576.
        // Over 50 iters (both A and B): 50×576×2 = 57,600 < 65,535 — safe without widening.
        // We do A+B in sequence (100 total stream iterations = 50 full rounds).
        size_t groups = safe_iters / 50;
        size_t remain = safe_iters % 50;
...
#define STREAM_TAIL(pi, bi, ei) \
        ...
        STREAM_TAIL(pa, ba, adj_end[10])
        STREAM_TAIL(pb, bb, adj_end[11])
        STREAM_TAIL(pc, bc, adj_end[12])
        STREAM_TAIL(pd, bd, adj_end[13])
        STREAM_TAIL(pe, be, adj_end[14])
        STREAM_TAIL(pf, bf, adj_end[15])
#undef STREAM_TAIL
    }
