// dp2_8s_fw_4acc_2560_32.cpp — 4 independent per-pair u16 accumulators + dual T1@2560+32.
// Combines 4acc independence (from 4acc_t0_64_448) with dual T1@2560+32 (from 2560_32).
// Unlike 4acc_t0_64_448 (T0@64+T1@448): no T0 hint, only dual T1@2560+32.
// Unlike 2560_32 (single acc): 4 independent accs break the 4-deep serial dep chain.
// Gap in the 4acc grid: 2048_32 and 3072_32 exist, 2560_32 does not.
// Overflow: per acc, 1 pair/iter × max 108/lane × 100 iters = 10,800 < 65,535.
...
// 4 independent per-pair accumulators + dual T1@2560+32 (no T0).
// acc0=streams(0,1), acc1=streams(2,3), acc2=streams(4,5), acc3=streams(6,7).
// Eliminates the 4-deep serial chain on acc_u16 from 2560_32.
// No T0 hint — unlike 4acc_t0_64_448, no near-prefetch competing for slots.
#define ITER_BODY \
    _mm_prefetch((const char*)(p0 + 2560), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p0 + 2560 + 32), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p1 + 2560), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p1 + 2560 + 32), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p2 + 2560), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p2 + 2560 + 32), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p3 + 2560), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p3 + 2560 + 32), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p4 + 2560), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p4 + 2560 + 32), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p5 + 2560), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p5 + 2560 + 32), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p6 + 2560), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p6 + 2560 + 32), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p7 + 2560), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p7 + 2560 + 32), _MM_HINT_T1); \
    { \
