// dp2_8s_nopf.cpp — dp2_8stream without SW prefetch.
// Hypothesis: each stream is sequential 64B/iter; hardware prefetcher handles it automatically.
// Removing 8 prefetch µops/iter frees decode bandwidth and reduces store-forwarding pressure.
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        while (__builtin_expect(p0 < s0 & p1 < s1 & p2 < s2 & p3 < s3 &
            ...
            // No SW prefetch — rely on hardware prefetcher (each stream is sequential).
