// HighLoad.fun — parse_integers  (VARIANT: avx2_8w_pf_dual)
// 8-window + dual-level prefetch: T2 at 6 iters (3072B) + T1 at 3 iters (1536B).
// Theory: two-stage memory pipeline — T2 moves data from DRAM→LLC 6 iters ahead,
// T1 moves data from LLC→L2 3 iters ahead. This prevents T1 from stalling on a
// cold LLC miss, since T2 has already warmed the LLC before T1 fires.
// On a VM with high DRAM latency, the T2 stage buys extra time for DRAM→LLC fill.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    while (p < safe_end) {
        // Dual-level prefetch: T2 (LLC) far, T1 (L2) near.
        // T2 warms LLC from DRAM 6 iters ahead; T1 moves LLC→L2 3 iters ahead.
        // This creates a two-stage pipeline: DRAM→LLC happens early enough that
        // the T1 stage sees a warm LLC hit instead of a cold DRAM fill.
        _mm_prefetch((const char*)(p + 3072),      _MM_HINT_T2);
        _mm_prefetch((const char*)(p + 3072 + 64), _MM_HINT_T2);
        _mm_prefetch((const char*)(p + 3072 + 128),_MM_HINT_T2);
        _mm_prefetch((const char*)(p + 3072 + 192),_MM_HINT_T2);
        _mm_prefetch((const char*)(p + 3072 + 256),_MM_HINT_T2);
        _mm_prefetch((const char*)(p + 3072 + 320),_MM_HINT_T2);
        _mm_prefetch((const char*)(p + 3072 + 384),_MM_HINT_T2);
        _mm_prefetch((const char*)(p + 3072 + 448),_MM_HINT_T2);
