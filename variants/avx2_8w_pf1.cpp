// HighLoad.fun — parse_integers  (VARIANT: avx2_8w_pf1)
// 8-window + T1 prefetch at 1 iteration (512 bytes) ahead.
// Rationale: On low-latency hardware (bare metal judge ~40-50ns DRAM),
// 1 iteration ahead (512B) suffices to cover L3→L2 fill latency.
// The champion (pf3) uses 3 iterations for high-latency cloud VMs.
// This variant is the missing grid point: T1@512B vs T0@512B and T1@1024B.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    while (p < safe_end) {
        // T1 (L2) at 1 iteration (512 bytes) ahead.
        // Low-latency hardware (judge bare metal ~40-50ns): 1 iter is enough.
