// HighLoad.fun — parse_integers  (VARIANT: avx2_cmov)
// avx2_parse3 (stride-32 + count-unroll) but with a reformed parse_num:
// the original uses a VARIABLE SHIFT (8*h where h∈{1,2}) and a multiply by
// (1+9*h2) — both are data-dependent. This variant instead precomputes BOTH
// possible chunk extractions (for len=9 and len=10) using FIXED shifts, then
// selects with cmov. Fixed shifts have lower latency (1c) than variable shifts
// (≥1c + CL-dependency chain), and the two fixed-shift operands are independent
// so they can execute in parallel. Same for the high-digit computation.
// The len<=8 slow path is kept but __builtin_expect'd unlikely.
#include <cstdio>
#include <cstdint>
#include <cinttypes>
#include <cstring>
...
// Reformulated parse_num: fixed shifts + cmov instead of variable shift + mul.
// For the 9/10-digit common path (95% of numbers), avoids the CL-based
// variable shift and the h2-dependent multiplier chain.
static inline uint64_t parse_num(const unsigned char* p, size_t len) {
    ...
    memcpy(&w0, p, 8);
    memcpy(&w1, p + 8, 8);
    // Compute both chunk options with FIXED shifts (parallel, no CL register):
    uint64_t chunk9  = (w0 >>  8) | (w1 << 56);   // h=1 (9-digit)
    uint64_t chunk10 = (w0 >> 16) | (w1 << 48);   // h=2 (10-digit)
    uint64_t is10 = (len == 10);
    uint64_t chunk = is10 ? chunk10 : chunk9;       // cmov, no branch
    // High digits: also two fixed paths
    uint64_t d0 = (w0 & 0xff) - '0';
    uint64_t d1 = ((w0 >> 8) & 0xff) - '0';
    uint64_t high9  = d0;                           // 9-digit: just the leading digit
    uint64_t high10 = d0 * 10 + d1;               // 10-digit: 2 leading digits
    uint64_t high = is10 ? high10 : high9;          // cmov
