// dp2_8s_fw_t1only_768.cpp — single-tier T1@768B only (no T0 prefetch).
// Tests whether T0@192B in the champion is actually helping or just wasting 8 µops/iter.
// On bare-metal judge (~80-100ns DRAM, ~7-8ns/iter), T1@768B=12 iters=84-96ns covers DRAM.
// T0@192B (3 iters≈21ns) pulls from L2→L1; if the HW prefetcher already does this
// from the T1-primed L2 data, T0 is just dead weight on the decode/issue pipeline.
// Removing T0: 8 prefetch µops/iter → 8 (half champion's 16). Might free decode BW.
// Identical to champion except no T0 prefetch lines in ITER_BODY.
...
// One iteration body: T0@192B (3 iters=24ns, L1) + T1@768B (12 iters=96ns, matches bare-metal ~80-100ns DRAM).
