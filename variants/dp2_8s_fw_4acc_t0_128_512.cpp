// dp2_8s_fw_4acc_t0_128_512.cpp — 4 independent per-pair u16 accumulators + T0@128+T1@512.
// 4acc version of the current champion (dp2_8s_fw_t0_128_512).
// T0@128B (2 iters ahead, covers L2→L1 at ~8ns/iter = 16ns).
// T1@512B (8 iters ahead, covers DRAM→L2 at ~80ns bare-metal = judge-tuned).
// 4acc reduces write-dependency chains vs single acc_u16, allowing out-of-order to better
// overlap the 4 acc adds with the 8 process_window_dp calls per iteration.
...
// 4 independent per-pair accumulators + T0@128B (near, L1) + T1@512B (judge-tuned DRAM→L2).
// 4acc = 4 independent acc_u16 registers to reduce write-dependency on the accumulator.
