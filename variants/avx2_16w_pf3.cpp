// avx2_16w_pf3: 16-window (1024 bytes/iter) + T1 prefetch at 3 iterations (3072 bytes) ahead.
// Theory: 16 independent nl_mask64 calls per outer loop = more MLP for DRAM.
// Prefetch distance scaled to match: 3 * 1024 = 3072 bytes ahead, 16 hints per iter.
#include <cstdio>
#include <cstdint>
#include <cinttypes>
#include <cstring>
    ...
    uint64_t m)
{
    ...
    if (__builtin_expect(cnt == 6, 1)) {
        ...
        const unsigned char *nl0=p+n0,*nl1=p+n1,*nl2=p+n2,*nl3=p+n3,*nl4=p+n4,*nl5=p+n5;
        sum += parse_quad(base,nl0-base,nl0+1,nl1-nl0-1,nl1+1,nl2-nl1-1,nl2+1,nl3-nl2-1)
        ...
        n0=__builtin_ctzll(mm); mm&=mm-1; n1=__builtin_ctzll(mm); mm&=mm-1;
        n2=__builtin_ctzll(mm); mm&=mm-1; n3=__builtin_ctzll(mm); mm&=mm-1;
        n4=__builtin_ctzll(mm); mm&=mm-1; n5=__builtin_ctzll(mm); mm&=mm-1;
        ...
        n0=__builtin_ctzll(mm); mm&=mm-1; n1=__builtin_ctzll(mm); mm&=mm-1;
        n2=__builtin_ctzll(mm); mm&=mm-1; n3=__builtin_ctzll(mm);
        const unsigned char *nl0=p+n0,*nl1=p+n1,*nl2=p+n2,*nl3=p+n3;
        sum += parse_quad(base,nl0-base,nl0+1,nl1-nl0-1,nl1+1,nl2-nl1-1,nl2+1,nl3-nl2-1);
        base = nl3+1;
        ...
        n2=__builtin_ctzll(mm); mm&=mm-1; n3=__builtin_ctzll(mm); mm&=mm-1;
        n4=__builtin_ctzll(mm); mm&=mm-1; n5=__builtin_ctzll(mm); mm&=mm-1;
        n6=__builtin_ctzll(mm); mm&=mm-1; n7=__builtin_ctzll(mm);
        const unsigned char *nl0=p+n0,*nl1=p+n1,*nl2=p+n2,*nl3=p+n3,
                            *nl4=p+n4,*nl5=p+n5,*nl6=p+n6,*nl7=p+n7;
        sum += parse_quad(base,nl0-base,nl0+1,nl1-nl0-1,nl1+1,nl2-nl1-1,nl2+1,nl3-nl2-1)
             + parse_quad(nl3+1,nl4-nl3-1,nl4+1,nl5-nl4-1,nl5+1,nl6-nl5-1,nl6+1,nl7-nl6-1);
        base = nl7+1;
    } else {
        while (m) {
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    uint64_t sum = 0;
    if (size < 1600) return scalar_tail(p, end, sum);
    const unsigned char* base = data;
    // 16 windows = 1024 bytes/iter; prefetch 3 full iters ahead = 3072 bytes
    const unsigned char* safe_end = end - 1088; // 16*64=1024 + 64 safety

    while (p < safe_end) {
        // Prefetch 3 iterations ahead (3072 bytes), 16 cache lines
        _mm_prefetch((const char*)(p + 3072),       _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 64),  _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 128), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 192), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 256), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 320), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 384), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 448), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 512), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 576), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 640), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 704), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 768), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 832), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 896), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 3072 + 960), _MM_HINT_T1);
        ...
        uint64_t m12 = nl_mask64(p + 768);
        uint64_t m13 = nl_mask64(p + 832);
        uint64_t m14 = nl_mask64(p + 896);
        uint64_t m15 = nl_mask64(p + 960);
        ...
        sum += process_window(p + 768, base, m12);
        sum += process_window(p + 832, base, m13);
        sum += process_window(p + 896, base, m14);
        sum += process_window(p + 960, base, m15);
        p += 1024;
    }
    while (p + 96 < end) {
