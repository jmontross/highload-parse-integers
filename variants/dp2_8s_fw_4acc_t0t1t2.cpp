// dp2_8s_fw_4acc_t0t1t2.cpp — triple-tier prefetch: T0@192B (L2→L1) + T1@1536B (DRAM→L2) + T2@8192B (DRAM→L3).
// Same T0+T1 as champion. Adds T2 (PREFETCHT2, fills L3) at 8192B = 128 iters ahead per stream.
// Theory: T2 primes L3 ahead of T1's DRAM→L2 fetch, creating a three-level pipeline:
//   DRAM → L3 (T2@8192B) → L2 (T1@1536B) → L1 (T0@192B).
// On slow VMs (~400ns DRAM), T2 at 128 iters = ~896ns ahead may reduce T1's effective DRAM latency.
// Overhead: 8 extra _mm_prefetch T2 calls per iteration (~8 extra µops, negligible vs compute).
...
// Three-tier prefetch per stream: T0@192B + T1@1536B (same as champion) + T2@8192B (new L3 tier).
