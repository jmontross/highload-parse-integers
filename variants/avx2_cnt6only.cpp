// HighLoad.fun — parse_integers  (VARIANT: avx2_cnt6only)
// Champion parse_quad logic with ONLY the cnt==6 fast path.
//
// HYPOTHESIS: The champion has cnt==4,5,6,7 fast paths. Each additional `else if`
// competes for branch predictor history entries. For 9-digit numbers, cnt==5,6,7
// each occur ~25-40% of the time, forcing the predictor to track multiple branches.
// By keeping ONLY cnt==6 and using while(m) for all other counts, the single branch
// "cnt==6 or not?" can be perfectly predicted, and the while(m) path for cnt!=6
// benefits from a highly predictable "no entry" pattern when cnt==6 dominates.
//
// Expected: slightly faster if branch prediction is the bottleneck,
//           slightly slower if parse_pair/parse_num call overhead matters.
#include <cstdio>
#include <cstdint>
#include <cinttypes>
#include <cstring>
