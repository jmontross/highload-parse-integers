// HighLoad.fun — parse_integers  (VARIANT: avx2_8w_pf5)
// 8-window + T1 prefetch at 3 iterations (2560 bytes) ahead.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    while (p < safe_end) {
        ...
        // T1 (L2) at 3 iterations (2560 bytes) ahead.
        // In VM environments with elevated effective latency, longer distance helps.
        _mm_prefetch((const char*)(p + 2560),      _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2560 + 64), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2560 + 128),_MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2560 + 192),_MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2560 + 256),_MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2560 + 320),_MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2560 + 384),_MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2560 + 448),_MM_HINT_T1);
