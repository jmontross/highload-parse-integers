// HighLoad.fun — parse_integers  (VARIANT: avx2_dual_cnt12)
// Combines the dual-window OOO trick (avx2_dual_window) with the
// full cnt==4..12 fast paths (avx2_cnt12).
//
// HYPOTHESIS:
//   avx2_dual_window's benefit: loading both nl_mask64 masks before
//   processing either window gives the OOO engine more ILP (~8 cycles
//   saved per 128-byte pair of windows).
//
//   avx2_cnt12's benefit: adding fast paths for cnt==11,12 eliminates
//   the serial while(m) fallback for those cases (rare but saves a
//   branch-heavy fallback path when it triggers).
//
//   avx2_dual_window lacks cnt==11,12 paths; avx2_cnt12 lacks the
//   dual-window outer loop. This variant combines both:
//   - outer loop: 2× nl_mask64 calls before processing either window
//   - process_window: handles cnt==4..12 explicitly (no while(m) fallback)
//
//   Expected result: within noise (already at STOP-FLOOR), but a clean
//   best-of-both combination worth measuring.
#include <cstdio>
#include <cstdint>
#include <cinttypes>
#include <cstring>
...
// Handles cnt==4..12 explicitly; while(m) fallback for cnt<4 or cnt>12.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    // Dual-window: load both masks before processing either window.
    // The two nl_mask64 loads are independent; OOO engine can issue them together.
    while (p < safe_end) {
