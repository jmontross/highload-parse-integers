// avx2_8w_pf4_i: interleaved champion with T1 prefetch at 2048B (4 iters ahead) instead of 1536B.
// Extends lookahead vs the champion's 3-iter / 1536B. Previously tested on non-interleaved variant
// (avx2_8w_pf4): best=0.1530s vs 0.1550s champion (Δ=1.3%, gate needs 1.5%, HOLD). Testing now
// on the interleaved base to see if the combination shows different behavior.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    while (p < safe_end) {
        // T1 prefetch at 2048B = 4 iterations ahead
