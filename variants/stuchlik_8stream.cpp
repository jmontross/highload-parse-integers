// HighLoad.fun — parse_integers (stuchlik_8stream)
// 8 spatially-separated independent memory streams (Change B from BREAKTHROUGH DIRECTIVE).
// Splits input into 8 equal blocks at newline boundaries; each block has its own
// independent base pointer. All 8 nl_mask64 loads are issued before any processing,
// creating 8 truly independent DRAM requests (~52 MB apart) vs. champion's single
// sequential stream. Documented path to rank-18 by zielaj / Stuchlik.
//
// Key difference from avx2_8w_pf3_interleaved (the champion):
//   Champion: 8 consecutive 64-byte windows at p, p+64, ..., p+448 (512-byte region,
//             single sequential DRAM stream; HW prefetcher sees it as 1 stream).
//   This:     8 windows from 8 blocks each ~52 MB apart (truly independent DRAM banks).
//             CPU line-fill-buffers (~10-16) can hold ALL 8 requests simultaneously.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    if (size < 8 * 800) return scalar_tail(data, end, sum);

    // Split into 8 equal blocks at newline boundaries.
    // adj_start[i] = first byte of first complete number in block i.
    // adj_end[i]   = adj_start[i+1] (exclusive; block i's last newline is at adj_end[i]-1).
    ...
    // Safe-end for each stream: leave 96 bytes margin for tail
    ...
    // Main loop: issue 8 independent loads (8 DRAM regions ~52 MB apart each),
    // then process all 8 windows. Repeat until any stream hits its safe end.
    while (__builtin_expect(p0 < s0 & p1 < s1 & p2 < s2 & p3 < s3 &
                            p4 < s4 & p5 < s5 & p6 < s6 & p7 < s7, 1)) {
        // Prefetch 1536 bytes ahead in each stream
        ...
        // Issue all 8 loads BEFORE any processing — 8 truly independent DRAM requests.
        // Each pair of AVX2 loads hits a different ~52 MB region of RAM.
        ...
        // Process all 8 windows. Each process_window updates only its own base[i].
        // No cross-stream dependency → OOO engine can execute all 8 in parallel.
        sum += process_window(p0, b0, m0); p0 += 64;
        sum += process_window(p1, b1, m1); p1 += 64;
        sum += process_window(p2, b2, m2); p2 += 64;
        sum += process_window(p3, b3, m3); p3 += 64;
        sum += process_window(p4, b4, m4); p4 += 64;
        sum += process_window(p5, b5, m5); p5 += 64;
        sum += process_window(p6, b6, m6); p6 += 64;
        sum += process_window(p7, b7, m7); p7 += 64;
    }

    // Per-stream cleanup (sequential scalar, at most a few windows each)
...
#else
// ARM / non-AVX2 fallback: same scalar as champion
