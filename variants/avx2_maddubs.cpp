// HighLoad.fun — parse_integers  (VARIANT: avx2_maddubs)
// avx2_64b + fastload parse_num + SIMD pair-parse via SSE PMADDUBSW.
//
// MOTIVATION: parse_8 has a 3-level tree of AND+SHR+MUL+ADD with ~14 cycles
// latency on the critical path. For 6 independent parse_8 calls per 64-byte
// block, the shared MUL port (1/cycle throughput) stagger-schedules them over
// ~18 cycles.  SSE PMADDUBSW can perform the same tree in VECTOR form, halving
// the per-number cost by processing TWO numbers simultaneously:
//
//   parse_pair(a, b):
//     1. Load last-8-digit tails of a and b into a 128-bit register (no shift)
//     2. _mm_maddubs_epi16(digits, [10,1,...]):   2-digit pairs  (1-cycle latency)
//     3. _mm_madd_epi16(l1, [100,1,...]):         4-digit values (1-cycle latency)
//     4. _mm_mullo_epi32(l2, [10000,1,...]):      scale high 4d (5-cycle latency)
//     5. _mm_hadd_epi32(l3, l3) → extract:       merge to 8d   (3-cycle latency)
//   Total: ~10 cycles for 2 numbers (instead of ~14 cycles × 2 = 28 cycles)
//   3 independent pairs per 64-byte block → all 3 overlap via OOO.
//
// Requires AVX2 (implies SSSE3 for MADDUBS, SSE4.1 for MULLO/EXTRACT).
#include <cstdio>
#include <cstdint>
#include <cinttypes>
#include <cstring>
...
// Scalar parse_num with fastload: avoids variable shifts for 9/10-digit case.
...
// Process two numbers in parallel using PMADDUBSW.
// Fast path: both numbers must have len >= 9 (caller guarantees for cnt==5/6).
    ...
    const unsigned char* __restrict__ p_b, size_t len_b)
{
    ...
    // Load the last 8 digit bytes of each number; pack into one 128-bit reg.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    while (p < safe_end) {
        ...
        if (__builtin_expect(cnt == 6, 1)) {
            ...
            // Three independent parse_pair calls; the compiler sees all three
            // as completely independent (no shared live state between them).
...
// Scalar simulation path for testing algorithm correctness on ARM.
