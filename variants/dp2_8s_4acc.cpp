// dp2_8s_4acc.cpp — 4 independent u16 accumulators (one per stream pair) to break
// the serial add_epi16 chain in the champion's hot loop. The champion uses ONE
// acc_u16 with 4 serial add_epi16 updates per inner iteration, creating a 4-cycle
// latency chain (each must wait for the prior result). With 4 INDEPENDENT accumulators,
// all 4 updates can execute in parallel (~1 cycle effective latency on ports 0/1).
// Merge the 4 at widen-time with a 2-level add tree (2 adds, not on the hot path).
// Inner=100 remains safe: max per lane per accum = max144 × 100 = 14,400 << 65,535.
// All other parameters identical to dp2_8s_fixed_widen (current champion).
...
// Merge 4 independent u16 accumulators and widen to wide_acc.
// max per lane per accum = 14,400 (inner=100 × max144) << 65,535.
// 4-way merge uses 2 add_epi16 pairs + 1 combine — only at widen frequency.
static __attribute__((noinline)) void
widen_4acc(__m256i& a0, __m256i& a1, __m256i& a2, __m256i& a3, uint64_t* wide_acc) {
    __m256i merged = _mm256_add_epi16(
        _mm256_add_epi16(a0, a1), _mm256_add_epi16(a2, a3));
    alignas(32) uint16_t lanes[16];
    _mm256_store_si256((__m256i*)lanes, merged);
...
// 4 independent accumulators: acc0=streams(0,1), acc1=streams(2,3),
//                              acc2=streams(4,5), acc3=streams(6,7).
// Each updates independently — no serial chain across the 4 adds.
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        // Double-loop: outer over widen-groups, inner exactly 100 iters.
        // Each accum receives at most max144 × 100 = 14,400 per lane < 65,535.
        // The 4 acc updates are INDEPENDENT (no shared register) → OOO can parallel.
