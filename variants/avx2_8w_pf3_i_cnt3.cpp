// HighLoad.fun — parse_integers  (VARIANT: avx2_8w_pf3_i_cnt3)
// Champion (avx2_8w_pf3_interleaved) + explicit cnt==3 fast path.
// The while(m) fallback handles cnt<4 and cnt>10. For uniform [0,2^31-1] input,
// P(cnt==3) ≈ 3.6% of 64-byte windows. Adding a fast path avoids 7 failed branch
// checks (cnt==6,5,7,4,8,9,10) and replaces 3 serial while(m) iterations with
// parse_pair + parse_num for those windows.
