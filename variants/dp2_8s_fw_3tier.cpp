// dp2_8s_fw_3tier.cpp — 3-tier prefetch: T0@192B + T1@1536B + T1@3072B per stream.
// Champion uses 2-tier: T0@512 + T1@3072 (16 uops/iter).
// This adds a MEDIUM T1 at 1536B to cover the gap between near (L1) and far (L2).
// Theory: judge bare-metal DRAM latency ~80-100ns and LLC→L2 ~30ns require
// both a medium-distance stage (T1@1536=24 iters) and a far-distance stage (T1@3072=48 iters).
// T0@192 (3 iters, 24ns) keeps data L1-warm right before use.
// Cost: 24 prefetch uops/iter vs champion's 16 — only worth it if 3rd stage hides latency.
...
// One iteration body: 3-tier T0@192B + T1@1536B + T1@3072B per stream.
// T0 fills L1 (3 iters ahead), T1 medium fills L2 (24 iters), T1 far fills L2 (48 iters).
// 24 prefetch uops/iter vs champion's 16.
