// avx2_8w_pf3_i2: 2-ahead interleaving (compute 2 masks before each process_window).
// Champion uses 1-ahead: compute m[i+1] while processing w[i].
// This variant computes m[i+1] AND m[i+2] before processing w[i], keeping 2 mask loads
// outstanding rather than 1. With 2 nl_mask64 calls in flight, the CPU can issue 4 loads
// (2 per nl_mask64 × 2) + 2 cmpeq/movemask before any window processing begins.
// On CPUs where load ports 2/3 saturate quickly, 2-ahead may be better than 1-ahead.
// Structure: m0,m1 → process(w0), m2 → process(w1), m3 → process(w2), ...
// (first 2 masks pre-computed, then 1-ahead for the rest to keep the pattern)
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    while (p < safe_end) {
        ...
        // 2-ahead: compute 2 masks before first process_window, then 1-ahead for rest.
        // This puts 2 outstanding vector loads (4 total AVX2 loads) in flight before
        // any integer parse work starts, helping the ROB see more MLP upfront.
