// dp2_8s_fw_400it.cpp — 400-iteration inner loop, 4acc, T0@512+T1@3072.
// Champion (200it) improved on 100it by halving widen_4acc overhead.
// This doubles again: 400 iters per group → 4× fewer widen calls vs 100it.
// Overflow: 400 × 108/lane = 43,200 < 65,535 per u16 lane ✓.
// T0@512 + T1@3072 unchanged (optimal on judge per prior sweeps).
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        // Double-loop: outer iterates widen groups, inner exactly 400 iters.
        // Per-acc overflow: 1 pair × max 108/lane × 400 iters = 43,200 < 65,535.
        size_t groups = safe_iters / 400;
        size_t remain = safe_iters % 400;
