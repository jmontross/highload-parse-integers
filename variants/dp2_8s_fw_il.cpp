// dp2_8s_fw_il.cpp — champion (dp2_8s_fw_3072_32) with TRUE 1-ahead interleaving.
// Champion computes ALL 8 masks → then processes ALL 8 windows (two separate phases).
// This variant: compute mask[i+1] while processing window[i] (strict 1-ahead).
// The OOO engine can overlap AVX2 loads (ports 2/3) with CTZ/PSHUF/ADD (ports 0/1/5)
// across adjacent stream pairs. Keeps double-loop + dual T1 prefetch at 3072+32.
// Differs from dp2_8s_interleaved (which uses old single-loop/iter_count structure
// and "inline nl_mask64 in arg" pattern — no explicit cross-stream overlap).
...
// One iteration body — 1-ahead interleaved: compute mask[i+1] while processing window[i].
// Phase order: 16 prefetches → m0,m1 → r0 (process w0) → m2 → r1 → m3 → r2 → ...
// AVX2 loads for m[i+1] (ports 2/3) can execute in parallel with CTZ/PSHUF for r[i] (ports 0/1/5).
#define ITER_BODY_IL \
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        // Safety: 100 × 4 × max144 = 57,600 < 65,535 ✓
