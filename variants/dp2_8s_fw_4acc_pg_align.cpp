// dp2_8s_fw_4acc_pg_align.cpp — page-aligned block starts.
// Instead of scanning forward from i*size/8 to next newline, align block starts
// to 4096-byte page boundaries. Ensures each stream starts on a page boundary,
// potentially improving TLB behavior and page-cache prefetch locality.
// Block split: skip to nearest 4KB-aligned boundary >= i*(size/8), then scan
// forward to next newline as before.
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        adj_start[0] = data;
        for (int i = 1; i < 8; i++) {
            // Align nominal split point to 4KB page boundary before scanning for newline.
            // This places each stream start on its own page, improving TLB locality.
            size_t nominal = (size_t)i * (size / 8);
            size_t pg_aligned = (nominal + 4095) & ~(size_t)4095;
            const unsigned char* q = data + (pg_aligned < size ? pg_aligned : nominal);
            while (q < end && *q != '\n') ++q;
