// HighLoad.fun — parse_integers  (VARIANT: avx2_8w_pf4)
// 8-window + T1 prefetch at 4 iterations (2048 bytes) ahead.
// Extends champion (avx2_8w_pf3 at 1536B) by one more iteration.
// Theory: VM DRAM latency may be higher than 3 iter × 159ns = 477ns.
// If effective L3 fill latency is ~636ns, 4 iters covers it fully.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    while (p < safe_end) {
        // T1 (L2) at 4 iterations (2048 bytes) ahead.
        // Increases prefetch lookahead over champion (1536B) to cover
        // higher VM memory latency if it exceeds ~477ns (3 iters).
        _mm_prefetch((const char*)(p + 2048),      _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2048 + 64), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2048 + 128),_MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2048 + 192),_MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2048 + 256),_MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2048 + 320),_MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2048 + 384),_MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2048 + 448),_MM_HINT_T1);
