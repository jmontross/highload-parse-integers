// dp2_8s_2w_fixed.cpp — 2 windows per stream + double-loop (outer=widen-groups,
// inner=50 fixed iters). Combines dp2_8s_2w's 16-outstanding-request structure
// with the champion's double-loop (no iter_count variable in the inner loop).
//
// dp2_8s_2w (single-loop + iter_count) exists but was tested only with single-loop.
// The double-loop structure eliminates 1 GPR and 1 conditional branch from the
// inner loop body, potentially improving scheduling and unrollability.
//
// Each inner iteration processes 8 streams × 2 windows = 16 loads, advancing
// 128B per stream. 16 outstanding cache-miss requests saturate the LFB.
//
// Overflow safety (single acc_u16): 8 pairs × max144 × 50 iters = 57,600 < 65,535 ✓
// Prefetch at 4096B per stream = 32 inner iterations ahead (128B/iter).
// Safe margin per stream: 160B (2×64+32 for double-window + AVX2 lookahead).
...
// 2 windows (128B) per stream per inner iteration.
// All 16 mask loads issued before any processing => 16 concurrent DRAM requests.
// NOTE: 2nd window (p+64) depends on base update from 1st window (p) within
// the SAME stream, but is INDEPENDENT of other streams => fine for OOO.
#define ITER_BODY_2W(PFD) \
    ...
    { \
    uint64_t m0a = nl_mask64(p0),       m0b = nl_mask64(p0 + 64); \
    uint64_t m1a = nl_mask64(p1),       m1b = nl_mask64(p1 + 64); \
    uint64_t m2a = nl_mask64(p2),       m2b = nl_mask64(p2 + 64); \
    uint64_t m3a = nl_mask64(p3),       m3b = nl_mask64(p3 + 64); \
    uint64_t m4a = nl_mask64(p4),       m4b = nl_mask64(p4 + 64); \
    uint64_t m5a = nl_mask64(p5),       m5b = nl_mask64(p5 + 64); \
    uint64_t m6a = nl_mask64(p6),       m6b = nl_mask64(p6 + 64); \
    uint64_t m7a = nl_mask64(p7),       m7b = nl_mask64(p7 + 64); \
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        // Each iter advances 128B per stream; need 160B safety margin.
        size_t safe_iters = (min_seg > 160) ? (min_seg - 160) / 128 : 0;
        ...
        // Double-loop: outer over widen-groups, inner exactly 50 iters.
        // 8 pairs per iter × max144 × 50 = 57,600 < 65,535 per u16 lane ✓
