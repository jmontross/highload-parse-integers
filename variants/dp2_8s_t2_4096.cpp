// dp2_8s_t2_4096.cpp — same as dp2_8s_pf4096 but uses _MM_HINT_T2 (LLC prefetch)
// instead of _MM_HINT_T1 (L2 prefetch). At 4096B ahead with 64B/iter, data is
// consumed ~64 iters later. L2 will have evicted it by then on a streaming
// workload; T2 fills into L3 which has higher capacity and stays hot longer.
// Rationale: 260MB L3 vs 420MB file = 62%% of data served from L3; T2 hint
// appropriate for data with L3-scale reuse distance.
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        for (size_t n = safe_iters; __builtin_expect(n > 0, 1); --n) {
            _mm_prefetch((const char*)(p0 + 4096), _MM_HINT_T2);
            _mm_prefetch((const char*)(p1 + 4096), _MM_HINT_T2);
            _mm_prefetch((const char*)(p2 + 4096), _MM_HINT_T2);
            _mm_prefetch((const char*)(p3 + 4096), _MM_HINT_T2);
            _mm_prefetch((const char*)(p4 + 4096), _MM_HINT_T2);
            _mm_prefetch((const char*)(p5 + 4096), _MM_HINT_T2);
            _mm_prefetch((const char*)(p6 + 4096), _MM_HINT_T2);
            _mm_prefetch((const char*)(p7 + 4096), _MM_HINT_T2);
