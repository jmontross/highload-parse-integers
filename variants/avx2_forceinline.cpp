// avx2_forceinline: avx2_parse_quad with __attribute__((always_inline)) on all helpers.
// Motivation: in the champion, cnt==5 and cnt==7 paths call parse_quad as a function
// (5 register pushes + 2 stack args = ~25 cycle overhead each call).
// cnt==7 calls parse_quad once (~35% of windows), cnt==5 calls parse_quad once (~20%).
// Force-inlining removes ~37ms of call overhead estimated at ~125M cycles total.
// cnt==6 was already inlined by clang; this extends that to all paths.
#include <cstdio>
#include <cstdint>
#include <cinttypes>
#include <cstring>
...
static inline __attribute__((always_inline)) uint32_t parse_8(uint64_t chunk) {
...
static inline __attribute__((always_inline)) uint64_t parse_num(const unsigned char* p, size_t len) {
...
static inline __attribute__((always_inline)) uint64_t scalar_tail(
    const unsigned char* p, const unsigned char* end, uint64_t sum) {
    uint64_t v = 0;
    for (; p < end; ++p) {
...
static inline __attribute__((always_inline)) uint64_t parse_pair(
...
static inline __attribute__((always_inline)) uint64_t parse_quad(
    const unsigned char* __restrict__ p_a, size_t len_a,
    const unsigned char* __restrict__ p_b, size_t len_b,
...
static inline __attribute__((always_inline)) uint64_t nl_mask64(const unsigned char* p) {
