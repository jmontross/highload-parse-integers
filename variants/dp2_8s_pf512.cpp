// dp2_8s_pf512.cpp — dp2_8stream with T1@512B SW prefetch (8 iterations ahead per stream).
// Shorter prefetch distance for judges with lower DRAM latency than this cloud VM.
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        while (__builtin_expect(p0 < s0 & p1 < s1 & p2 < s2 & p3 < s3 &
            ...
            // SW prefetch: 512B ahead per stream (8 iterations of 64B)
            _mm_prefetch((const char*)(p0 + 512), _MM_HINT_T1);
            _mm_prefetch((const char*)(p1 + 512), _MM_HINT_T1);
            _mm_prefetch((const char*)(p2 + 512), _MM_HINT_T1);
            _mm_prefetch((const char*)(p3 + 512), _MM_HINT_T1);
            _mm_prefetch((const char*)(p4 + 512), _MM_HINT_T1);
            _mm_prefetch((const char*)(p5 + 512), _MM_HINT_T1);
            _mm_prefetch((const char*)(p6 + 512), _MM_HINT_T1);
            _mm_prefetch((const char*)(p7 + 512), _MM_HINT_T1);
