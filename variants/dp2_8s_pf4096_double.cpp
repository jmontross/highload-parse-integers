// dp2_8s_pf4096_double.cpp — champion (dp2_8s_pf4096) + dual T1 prefetch per stream.
// Extends pf_double (3072B+64B, HOLD 2026-07-09) to the champion's 4096B distance.
// pf_double@3072B showed consistent 1.0-1.25% improvement over 3072B single-prefetch
// but never cleared the 1.5% gate. Applying the same dual-CL coverage at 4096B.
//
// Rationale: streams start at unaligned offsets (adj_start[i] = byte after '\n').
// Each p_i advances by 64B/iter, maintaining a fixed misalignment relative to 64B
// boundaries. When p_i mod 64 != 0, the 64B nl_mask64 window straddles two cache
// lines; a single prefetch at p_i+4096 covers only the first. The second prefetch
// at p_i+4096+64 covers the adjacent cache line, ensuring both halves of every
// future window are warm in L2 before we access them.
//
// Cost: 16 prefetch µops/iter vs 8 in champion — 8 extra PREFETCHT1 instructions.
// Prefetch throughput: 2/cycle on modern CPUs; 16 prefetches = ~8cy vs 8 = ~4cy.
// With ~250cy DRAM latency per 64B iteration, the extra 4cy is <2% overhead.
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        for (size_t n = safe_iters; __builtin_expect(n > 0, 1); --n) {
            // Dual prefetch: covers both cache lines of the 64B window at 4096B ahead.
            // When p_i is misaligned to 64B boundaries, p_i+4096 to p_i+4096+63
            // spans two cache lines; a single prefetch covers only the first.
            _mm_prefetch((const char*)(p0 + 4096),    _MM_HINT_T1);
            _mm_prefetch((const char*)(p0 + 4096+64), _MM_HINT_T1);
            _mm_prefetch((const char*)(p1 + 4096),    _MM_HINT_T1);
            _mm_prefetch((const char*)(p1 + 4096+64), _MM_HINT_T1);
            _mm_prefetch((const char*)(p2 + 4096),    _MM_HINT_T1);
            _mm_prefetch((const char*)(p2 + 4096+64), _MM_HINT_T1);
            _mm_prefetch((const char*)(p3 + 4096),    _MM_HINT_T1);
            _mm_prefetch((const char*)(p3 + 4096+64), _MM_HINT_T1);
            _mm_prefetch((const char*)(p4 + 4096),    _MM_HINT_T1);
            _mm_prefetch((const char*)(p4 + 4096+64), _MM_HINT_T1);
            _mm_prefetch((const char*)(p5 + 4096),    _MM_HINT_T1);
            _mm_prefetch((const char*)(p5 + 4096+64), _MM_HINT_T1);
            _mm_prefetch((const char*)(p6 + 4096),    _MM_HINT_T1);
            _mm_prefetch((const char*)(p6 + 4096+64), _MM_HINT_T1);
            _mm_prefetch((const char*)(p7 + 4096),    _MM_HINT_T1);
            _mm_prefetch((const char*)(p7 + 4096+64), _MM_HINT_T1);
