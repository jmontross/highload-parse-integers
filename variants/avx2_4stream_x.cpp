// avx2_4stream_x.cpp — 4 streams, 2 windows/stream, CROSS-INTERLEAVED processing
// Same structure as avx2_4stream but with crossed window processing order:
//   process all 4 streams' FIRST windows, then all 4 streams' SECOND windows
// (vs avx2_4stream which processes BOTH windows of stream 0 before stream 1)
//
// WHY CROSSING MATTERS: process_window has an intra-stream serial dependency:
//   stream[i].window_b.base depends on stream[i].window_a's last newline.
// By inserting 4 independent process_window calls (streams 1,2,3 window a)
// between stream 0's window a and window b, the OOO engine has 4 independent
// operations to execute while waiting for stream 0's serial base update.
// This turns a serial-pair bottleneck into a parallel-4 throughput opportunity.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    while (__builtin_expect(p0 < s0 & p1 < s1 & p2 < s2 & p3 < s3, 1)) {
        ...
        // Compute all 8 masks (issue all loads before processing)
        ...
        // CROSSED processing order: all 4 streams' first windows, then second windows.
        // The serial base-dep chain for stream 0: window_a → window_b.
        // With 3 independent stream process_window calls between them, OOO can fill
        // the serial latency gap (~30 cycles for parse_quad) with other streams' work.
        sum += process_window(p0,      b0, m0a);
        sum += process_window(p1,      b1, m1a);
        sum += process_window(p2,      b2, m2a);
        sum += process_window(p3,      b3, m3a);
        // Now each stream's base is ready for the second window:
