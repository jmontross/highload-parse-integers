// HighLoad.fun — parse_integers  (VARIANT: avx2_6window)
// Process 6 consecutive 64-byte windows per outer-loop iteration.
//
// Rationale: 5-window was HOLD (0.2260s, tied champion best today),
// 8-window regressed today (0.2370s vs champion 0.2140s) due to
// I-cache/ROB pressure. 6 windows is the untested midpoint: enough
// MLP to hide load latency without the code-size penalty of 8 windows.
//
// Uses the full cnt==4..12 process_window from avx2_cnt12 (factored
// into process_window_cnt12 here) so the cnt dispatch is compact.
// safe_end offset: 6×64 + 32 safety = 416 bytes before end.
#include <cstdio>
#include <cstdint>
#include <cinttypes>
#include <cstring>
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    const unsigned char* safe_end = end - 416; // 6 windows (384) + safety (32)

    // 6-window loop: load 6 masks before processing any window.
    // Midpoint between 5-window (HOLD today) and 8-window (slower today).
    while (p < safe_end) {
