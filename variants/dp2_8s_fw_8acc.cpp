// dp2_8s_fw_8acc.cpp — 8 independent per-stream u16 accumulators (no cross-stream pair-add).
// Champion dp2_8s_fw_4acc combines pairs before widening: cvt(r0+r1)->acc0, cvt(r2+r3)->acc1, ...
// This eliminates the _mm_add_epi8 step by keeping 8 separate 256-bit accumulators.
// Each stream's result widens directly: acc0+=cvt(r0), acc1+=cvt(r1), ..., acc7+=cvt(r7).
// Overflow: each acc gets max 9/lane/iter × 100 iters = 900 < 65535. Safe to widen every 100 iters.
// Tradeoff: 8 YMM accumulators vs 4, but frees 4 _mm_add_epi8 instructions from the hot loop.
// Same prefetch as champion: T0@192B + T1@1536B.
...
widen_8acc(__m256i& a0, __m256i& a1, __m256i& a2, __m256i& a3,
           ...
           uint64_t* wide_acc) {
    __m256i sum01 = _mm256_add_epi16(a0, a1);
    __m256i sum23 = _mm256_add_epi16(a2, a3);
    __m256i sum45 = _mm256_add_epi16(a4, a5);
    __m256i sum67 = _mm256_add_epi16(a6, a7);
    __m256i sum = _mm256_add_epi16(_mm256_add_epi16(sum01, sum23),
                                   _mm256_add_epi16(sum45, sum67));
...
// 8 independent per-stream u16 accumulators: T0@192B + T1@1536B per stream.
// No cross-stream pair-add in hot loop — eliminates 4 _mm_add_epi8 instructions.
#define ITER_BODY(PFD) \
    ...
    acc0 = _mm256_add_epi16(acc0, _mm256_cvtepu8_epi16(r0)); \
    acc1 = _mm256_add_epi16(acc1, _mm256_cvtepu8_epi16(r1)); \
    acc2 = _mm256_add_epi16(acc2, _mm256_cvtepu8_epi16(r2)); \
    acc3 = _mm256_add_epi16(acc3, _mm256_cvtepu8_epi16(r3)); \
    acc4 = _mm256_add_epi16(acc4, _mm256_cvtepu8_epi16(r4)); \
    acc5 = _mm256_add_epi16(acc5, _mm256_cvtepu8_epi16(r5)); \
    acc6 = _mm256_add_epi16(acc6, _mm256_cvtepu8_epi16(r6)); \
    acc7 = _mm256_add_epi16(acc7, _mm256_cvtepu8_epi16(r7)); \
    }
