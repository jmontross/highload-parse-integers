// HighLoad.fun — parse_integers  (VARIANT: avx2_8w_i_nopf)
// Champion (avx2_8w_pf3_interleaved) with ALL software prefetch removed.
// Tests whether the HW prefetcher can handle 8 sequential 64-byte streams
// without explicit SW hints. Previous tests showed pf was necessary, but
// VM memory subsystem state changes run-to-run. If this is competitive,
// the judge (lower-latency DRAM ~70ns, fewer streams to track) might not
// need SW prefetch at all — saving 8 prefetch instructions per outer iter.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    while (p < safe_end) {
        // No SW prefetch: test HW prefetcher alone on 8 sequential streams.
