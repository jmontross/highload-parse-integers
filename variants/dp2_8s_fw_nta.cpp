// dp2_8s_fw_nta.cpp — double-loop with NTA (non-temporal) prefetch at 4096B.
// Like champion (dp2_8s_fixed_widen, T1@4096B) but uses _MM_HINT_NTA instead of
// _MM_HINT_NTA. NTA (prefetchnta) brings data to L1D with "non-temporal" policy:
// avoids pollution of L2/L3 caches. For our streaming workload (each byte processed
// once, ~400MB file >> L3), NTA avoids unnecessary cache pollution and may improve
// throughput by reducing eviction pressure. T1 fills into L2 (~512KB) and we
// process data before it's evicted; NTA bypasses L2, reducing cache pressure.
// Genuinely untested: all previous prefetch experiments used T1 or T2 hints.
...
#define ITER_BODY(PFD) \
    _mm_prefetch((const char*)(p0 + (PFD)), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p1 + (PFD)), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p2 + (PFD)), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p3 + (PFD)), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p4 + (PFD)), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p5 + (PFD)), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p6 + (PFD)), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p7 + (PFD)), _MM_HINT_NTA); \
    { \
