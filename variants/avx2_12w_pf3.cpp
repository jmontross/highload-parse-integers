// HighLoad.fun — parse_integers  (VARIANT: avx2_12w_pf3)
// 12 windows per iteration + T1 prefetch at 3 iterations (2304 bytes) ahead.
//
// avx2_12window was HOLD without prefetch (0.172s vs 8-window 0.163s).
// avx2_8w_pf3 champion uses T1@1536B to great effect.
// This applies the same prefetch trick to the 12-window variant:
// 3 × 768 = 2304 bytes ahead, 12 prefetch calls covering the next iteration.
// If 12 windows has more MLP to expose, prefetch may push it past 8-window+pf3.
//
// 12 windows = 768 bytes/iter; 3 iters ahead = 2304 bytes.
...
static uint64_t solve(const unsigned char* data, size_t size) {
    ...
    // 12-window loop + T1 prefetch 3 iterations (2304 bytes) ahead.
    while (p < safe_end) {
        // Prefetch the next-iteration's 768 bytes into L2, 3 iters ahead.
        _mm_prefetch((const char*)(p + 2304),       _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2304 +  64), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2304 + 128), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2304 + 192), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2304 + 256), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2304 + 320), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2304 + 384), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2304 + 448), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2304 + 512), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2304 + 576), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2304 + 640), _MM_HINT_T1);
        _mm_prefetch((const char*)(p + 2304 + 704), _MM_HINT_T1);
