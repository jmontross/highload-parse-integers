// HighLoad.fun — parse_integers  (VARIANT: avx512_nl_8w)
// Champion avx2_8w_pf3_interleaved with ONE change: nl_mask64 uses a single
// _mm512_cmpeq_epi8_mask instruction (AVX-512BW) instead of 2×vmovemask+or (5 µops).
//
// On Sapphire Rapids (this machine: avx512_vnni, amx_tile, avx512_fp16) there is
// NO AVX-512 frequency downscaling penalty (that only affected Cascade Lake / SKX).
//
// In the 8-window loop, 8 × nl_mask64 = 8 × 7 µops = 56 µops (AVX2 path).
// With AVX-512: 8 × 2 µops = 16 µops → saves 40 µops per 512-byte iteration.
// Rough estimate: ~15-20% speedup on the scan portion (if not bandwidth-limited).
//
// ARM fallback: uses AVX2 path unchanged.
#include <cstdio>
#include <cstdint>
#include <cinttypes>
#include <cstring>
...
#ifdef __AVX512BW__
    // ONE instruction: load 64 bytes + compare for newlines → 64-bit mask.
    // On Sapphire Rapids there is no AVX-512 frequency downscaling (unlike Cascade Lake/SKX).
