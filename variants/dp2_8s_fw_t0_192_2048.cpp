// dp2_8s_fw_t0_192_2048.cpp — double-loop + two-tier prefetch.
// T0@192B (3 iters ahead, fills L1) + T1@2048B (32 iters ahead, fills L2).
// Champion uses T1@1536B (24 iters). Longer far-tier: 2048B = 32 iters covers VMs
// with slightly higher DRAM latency than judge estimate. Tests if more DRAM headroom
// helps on this VM without excess uop pressure (same 16 prefetch µops/iter).
...
// One iteration body: T0@192B (near, L1) + T1@2048B (far, L2) per stream.
// Longer T1 than champion: 32 iters × 8ns/iter = 256ns, more margin for slower DRAM.
