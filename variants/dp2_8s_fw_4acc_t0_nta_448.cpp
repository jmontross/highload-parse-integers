// dp2_8s_fw_4acc_t0_nta_448.cpp — 8 streams, T0=NTA@64B + T1@448B, 4 accumulators.
// Hypothesis: PREFETCHNTA for the immediate next cache line (T0 range) bypasses L2/L3,
// keeping those caches free for T1 data. Sequential per-stream access means each cache line
// is read once; NTA's "no L2/L3 pollution" property should reduce cache pressure vs T0.
// Result vs champion: if NTA frees LFB/cache bandwidth for T1, this should be faster.
// If HW prefetcher already covers sequential access just as well, NTA adds no benefit.
...
// 8 streams + T0=NTA@64B (bypasses L2/L3, reduces cache pollution) + T1@448B.
// NTA = non-temporal: data loaded into L1 only, evicted directly without L2 staging.
// For sequential once-only access, NTA avoids polluting L2/L3 with T0 data,
// leaving cache hierarchy available for T1 traffic.
#define ITER_BODY(PFD) \
    _mm_prefetch((const char*)(p0 + 64), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p0 + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p1 + 64), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p1 + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p2 + 64), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p2 + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p3 + 64), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p3 + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p4 + 64), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p4 + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p5 + 64), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p5 + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p6 + 64), _MM_HINT_NTA); \
    _mm_prefetch((const char*)(p6 + (PFD)), _MM_HINT_T1); \
    _mm_prefetch((const char*)(p7 + 64), _MM_HINT_NTA); \
