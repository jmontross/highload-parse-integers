// HighLoad.fun — parse_integers  (VARIANT: avx512_maddubs)
// avx2_maddubs champion + AVX-512BW newline scan.
//
// The sole change vs avx2_maddubs: nl_mask64 uses a SINGLE 512-bit
// compare+mask instruction (_mm512_cmpeq_epi8_mask) instead of
//   2× _mm256_loadu + 2× _mm256_cmpeq + 2× _mm256_movemask + OR
// saving ~5 instructions (~5 cycles) per 64-byte block = ~6% of total work.
// Parsing path (parse_pair, cnt==6/5 fast paths) is unchanged.
//
// Falls back to AVX2 nl_mask64 when __AVX512BW__ is not defined so the
// file compiles anywhere.
#include <cstdio>
#include <cstdint>
#include <cinttypes>
#include <cstring>
    ...
    const unsigned char* __restrict__ p_b, size_t len_b)
{
    ...
    // Use shuffle+add instead of hadd: PSHUFD is 1-cycle vs PHADDD 3-cycle.
    // shuffle(l3, 0x88) = [l3[0], l3[2], l3[0], l3[2]] = [a_hi, b_hi, ...]
    // shuffle(l3, 0xdd) = [l3[1], l3[3], l3[1], l3[3]] = [a_lo, b_lo, ...]
...
// AVX-512BW: 64 bytes in ONE instruction; else fall back to two AVX2 loads.
