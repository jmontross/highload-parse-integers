// dp2_8s_fw_4acc_t0_64_448_200it.cpp — champion config + 200 inner iterations (vs 100).
// Champion: T0@64B+T1@448B, 4 independent per-pair u16 accumulators, inner=100.
// This variant: SAME prefetch (T0@64+T1@448) + SAME 4acc structure, but inner=200.
// Theory: halving widen_4acc calls reduces branch overhead; 8-stream bandwidth bound.
// Overflow: each accum gets 1 pair/iter × max 108/lane × 200 iters = 21,600 < 65,535.
// First test of champion's exact prefetch config at 200 iters (all prior 200it variants
// used T0@512+T1@3072, a completely different prefetch distance).
...
static uint64_t solve(const unsigned char* data, size_t size) {
...

    {
        ...
        // Double-loop: outer iterates widen groups, inner exactly 200 iters.
        // Per-acc overflow: 1 pair × max 108/lane × 200 iters = 21,600 < 65,535.
        size_t groups = safe_iters / 200;
        size_t remain = safe_iters % 200;
